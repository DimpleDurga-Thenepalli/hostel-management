...
function ParentOutingsPage() {
  ...
  const load = async () => {
    const res = await api.get('/parent/outings');
    setOutings(res.data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.post('/parent/outings', form);
      setForm({ reason: '', fromDate: '', toDate: '' });
      setMessage('Outing request submitted and parent-approved.');
      await load();
    } catch (err) {
      console.error(err);
  ...
  const handleApprove = async (id) => {
    await api.patch(`/parent/outings/${id}/approve`);
    await load();
  };
  ...
  return (
    <div>
      ...
      <p>Approve or create outing requests for your ward.</p>
...
export default ParentOutingsPage;
